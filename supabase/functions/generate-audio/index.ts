...
async function pollForResult(endpoint: string, requestId: string, apiKey: string): Promise<any> {
  for (let i = 0; i < 120; i++) {
    ...
    if (statusData.status === "COMPLETED") {
      ...
      try { return JSON.parse(resultText); } catch { throw new Error("Invalid response from AI service"); }
    }
...
serve(async (req) => {
  ...
  try {
    ...
    const { prompt, model_id = "stable-audio", cauris_cost = 0, project_id, ...rawSettings } = body;
    ...
    if (!endpoint) {
      return new Response(JSON.stringify({ error: "Modèle audio inconnu" }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
    ...
    if (!submitResp.ok) {
      ...
      return new Response(JSON.stringify({ error: "Erreur du service de génération audio. Réessayez." }), { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
    ...
    if (!audioUrl) {
      ...
      await adminClient.rpc("add_cauris", { p_user_id: userId, p_amount: cost });
      return new Response(JSON.stringify({ error: "Aucun audio généré" }), { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
    ...
    try {
      ...
      await adminClient.from("generations").insert({
        user_id: userId, prompt: prompt.slice(0, 5000), image_url: publicUrl, media_type: "audio", project_id: project_id || null,
      });
      return new Response(
        JSON.stringify({ audio_url: publicUrl, new_balance: deductResult }),
        { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } },
      );
    } catch (err) {
      console.error("Storage upload error:", err);
      return new Response(JSON.stringify({ audio_url: audioUrl, new_balance: deductResult }), { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
  } catch (e) {
