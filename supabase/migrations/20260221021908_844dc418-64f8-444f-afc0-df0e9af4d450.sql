...
-- Fix add_cauris: allow service role (auth.uid() IS NULL) to call it
...
BEGIN
  ...
  -- but block regular users from modifying other users' credits
...
-- Fix deduct_cauris: same pattern for edge function refunds
